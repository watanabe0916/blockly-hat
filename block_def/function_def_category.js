...
Blockly.Hat.def_func = function (block) {
    ...
    //先行評価のとき
    if(dropdown_code === "1"){
        def_array = define.split('  ');
        for(var i = 0;i < def_array.length;i++){
            if(i >= 3 && i < def_array.length - 2){// 引数の部分だけ修正
                def_array[i] = ' ' + def_array[i] + '^(' + def_array[i] + ')';
            }
            redefine += def_array[i];
        }
        return "(defineCPS"+ redefine + do_code + "\n " + return_code + ")\n";
    }

    return "(defineCPS"+ define + do_code + "\n " + return_code + ")\n";
};
